...
\textbf{Gruppen}\newline
Eine Gruppe ist eine fundamentale algebraische Struktur, eine Gruppe besteht aus einer Menge von Elementen und einer zweistelligen Verknüpfung. Für alle Elemente müssen die sogenannten Gruppenaxiome gelten.   
\begin{definition}
...
Das Konzept von einer zyklischen Gruppe ist von großer Bedeutung für den AKS-Primzahltest, daher ist es sinnvoll eine Formale Definition zu haben. Um dieses Konzept zu definieren und zu verdeutlichen betrachtet man zunächst die Potenzen\footnote{Potenzen hier sind nicht wirklich Potenzen. Für eine Gruppe $(G,\circ)$ sind Potenzen einfach eine wiederholte Anwendung der Operator $\circ$, beispielsweise $a^n = \underbrace{a \circ a \circ \dots a }_{n- mal}$ } der Elemente von beliebigen Gruppen. Sei zunächst $(G,\circ)$ eine Gruppe und $a \in G$ beliebig. Wir betrachten die Menge $\langle a \rangle = \{ a^i \mid i \in \mathbb{Z}\}$, diese Menge heißt die Menge der Potenzen von $a \in G$. $G$ heißt zyklisch, wenn sie ein Element $a$ enthält, sodass $\langle a \rangle = G$. 
...
\textbf{Struktur und Eigenschaften von zyklischen Gruppen}\newline
Man unterscheidet zwischen endlichen und unendlichen zyklischen Gruppen, für den AKS-Primzahltest sind endliche zyklische Gruppen interessant. Daher werden hier nur die Struktur und die Eigenschaften der endlichen zyklischen Gruppen diskutiert.

\begin{theorem}
Seien $G$ eine Gruppe und $a \in G$ mit $ord_{G}(a) = m, \, m \geq 1$ dann gilt:\newline
\begin{enumerate}
    \item $\langle a \rangle = \{e,a,a^2, \dots,a^{m-1} \}$.
    
    \item $a^i = a^j \iff m \mid j - i$.
\end{enumerate}
\end{theorem}
\textbf{Ringe}\newline
...
Sei $R$ ein kommutativer Ring und $f \in R[X]$ ein Polynom, sodass $f \neq 0$. Ferner seien $a_1, a_2,...,a_n \in R$ die Nullstellen/Wurzeln von $f$ mit $a_i - a_j \in R\setminus\{0\} , \, \forall i,j, \, 1 \leq i,j, \leq n$. Dann ist $deg \, f \geq n$.
\end{theorem}
...
Der Binomialkoeffizient spielt eine große Rolle für den Korrektheitsbeweis des AKS-Algorithmus, insbesondere bei Abschätzungen von Gruppen, die für den Beweis nötig sind.
\begin{flushleft}
...
\textbf{IA:} Für $n = 1$ gibt es genau $k$ Möglichkeiten, um eine Zahl auszuwählen, sodass diese Zahl kleiner oder gleich $k - 1$ ist. Außerdem gilt auch ${n + k - 1 \choose n} = {k \choose 1} = k$.\newline  
...
seien $a,n \in \mathbb{N}$ mit a < n und teilerfremd\footnote{. Das heißt $(a,n) = 1.$}, dann ist n genau dann eine Primzahl, wenn \newline
...
Der 5. Schritt des Algorithmus überprüft $\ell$ Gleichungen. Wir wollen nun die Ausgabe PRIME untersuchen, also nach Voraussetzung ist die Ausgabe PRIME . Das heißt, für alle Gleichungen $\ell$ gilt:\newline\newline
\centerline{$(X + a)^n = X^n + a $ (mod $X^r - 1, n$)}.
...
Zur Definition der zweiten Gruppe sei nun $h(X)$ einer dieser Teiler aus Lemma \ref{cyc_ov_fields}. Die zweite Gruppe $\mathcal{G}$ sei die multiplikative Gruppe aller Restklassen von Polynomen in $\mathcal{P}$ modulo $h(X)$ und $p$, das bedeutet $\mathcal{G} = \{f \; (mod \, h(X), p) \mid f \in \mathcal{P} \}$. $\mathcal{G}$ ist eine Untergruppe von $\mathbb{K} = \mathbb{K}_p[X] / (h(X))$ und wird von $X,X+1,X+2, ..., X + \ell $ im Körper $\mathbb{K}$ erzeugt.\newline
...
\end{align*}
Zahlen und folglich existieren mindestens ${t + \ell \choose t - 1}$ Polynome vom Grad $ < t $ in $\mathcal{G}$.\newline Daraus folgt $| \mathcal{G}| \geq {t + \ell \choose t - 1} $ 
\end{proof}

Wenn $n$ keine Potenz von $p$ ist, erhält man auch eine obere Grenze für $|\mathcal{G}|$. Mit dieser oberen Schranke kann man später einen Widerspruch zu konstruieren. 
...
\begin{proof}
Wir betrachten folgende Teilmenge von $I$:
\begin{equation}
...
&\geq n^{\sqrt{t}}.
\end{align*}
Gemäß Lemma \ref{closing_lemma} gilt $|\mathcal{G}| \leq n^{\sqrt{t}}$, falls $n$ keine Potenz von $p$ ist, daraus kann man folgern, dass $n = p^k$, für ein $k \geq 1$. Aber die Ausgabe ist nach Voraussetzung PRIME\footnote{. Die Ausgabe ist Nach Voraussetzung eine Primzahl.}, folglich gilt $ k= 1 \Rightarrow n = p$. Somit ist $n$ eine Primzahl.
\end{proof}
...
In diesem Kapitel werden die Ergebnisse aus dem vierten Kapitel experimentell geprüft. Zuerst die Laufzeiten der im Kapitel 4 untersuchten Algorithmen(Schritt 1, 2) experimentell geprüft. Des Weiteren wird geprüft welchen Einfluss die Primalität einer Zahl auf die Laufzeit hat. Das heißt es wird auf folgende Frage benatowertet:\newline
...
\begin{tabular}{ |p{3cm}||p{3cm}|p{3cm}|p{3cm}| |p{1.5cm}| }
 ...
 \hline
 Zahl & Bits(n)&Ergebnis&Zeit in s & $n = a^b $\\
 \hline
...
Einer der Hauptschritte des AKS-Algorithmus ist der zweite Schritt. Im zweiten Schritt wird nach einem geeinigten $r$ gesucht, dies passiert durch ausprobieren von mehreren $r$ Werten. In Lemma \ref{limit_of_r} wurde gezeigt, dass dieses $r$ existiert, es wurde außerdem gezeigt, dass $r \leq log^5 n$. Nun soll getestet werden, wie lange es dauert, um dieses geeignete $r$ zu finden.  
\begin{table}[h!]
\centering
\begin{tabular}{ |p{3cm}||p{3cm}|p{3cm}|p{3cm}|  }
 ...
 \hline
 Zahl & Bits(n) &Ergebnis&Zeit in s\\
 \hline
 7   & 3    &5&   $0.000032$\\
 41&   6  & 41   &$0.00038$\\
 771 &10 & 3&  $0.0001$\\
