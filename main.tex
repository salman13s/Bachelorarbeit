...
Sei $n \in \mathbb{N}$ mit n > 1. Die Eulersche Phi-Funktion wird mit $\phi(n)$ bezeichnet, ist die Anzahl an Zahlen zwischen n und 0, die Teilerfremd sind. Außerdem ist $|\mathbb{U}_{n}| = \phi(n)$. Sei nun $n =p_{1}^{e_{1}}p_{2}^{e_{2}}...p_{k}^{e_{k}}$ die Primzahlfaktorisierung von n, dann ist die Eulersche Phi-Funktion folgendermaßen definiert:\newline

\centerline{\large $\phi(n) = n \prod_{i=1}^{k} (1 - \frac{1}{p_{i}})$}
\end{definition}
\smallskip
% Theoreme
...
\begin{proof}
Induktion: Für n = 2 ist es leicht zu sehen, dass ${5 \choose 2} > 2^3$. Sei nun das obere Theorem für ein beliebiges $k > 2 $, mit $ k \in \mathbb{N}$ erfüllt.\newline\newline
...
\begin{equation}
     \frac{(2k + 1)!}{(k + 1)! \cdot k!} \cdot \frac{(2k + 2) \cdot (2k + 3)}{(k + 2) \cdot (k + 1)} > 2^{k+1} \cdot 2.\newline\newline
\end{equation}
...
\begin{equation}
    \frac{(2k + 2) \cdot (2k + 3)}{(k + 2) \cdot (k + 1)} = \frac{2 \cdot (2k + 3 )}{k + 2} > 2
\end{equation}
...
\centerline{\large $(a + b)^n  = \sum_{k=1}^n {n \choose k} a^k b^{n-k}$}
\end{theorem}
\end{flushleft}
...
\end{equation}
\newline\newline
Im Nenner lässt sich q! als $q \cdot (q-1)!$ schreiben und im Zähler lässt sich n als $q^k\cdot m$ schreiben, $m \in \mathbb{Z}_{+}$. Das q im Nenner hebt sich mit einem der qs im Zähler auf. Der resultierende Term ist daher nicht durch $q^k$ teilbar, außerdem sind $q^k$ und $a^{n-k}$ teilerfremd. Das heißt $(X + a)^n \neq X^n + a $(mod n).
\end{proof}
...
%STEP 5
\item \textbf{for} a = 1 to $\lfloor \sqrt{\phi(r)}log n $:
%STEP 6
\item  \textbf{if}$(X + a)^n \neq X^n + a $(mod )
